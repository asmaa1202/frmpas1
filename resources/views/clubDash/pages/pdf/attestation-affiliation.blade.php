...
<head>
    ...
    <title>ATTESTATION D'AFFILIATION</title>
...
<p style="font-size: 16px; text-indent: 15px;">Je, soussigné, <span style="font-weight: bold;">{{ $clubPresident ?? '--' }}</span>, Président de la Fédération Royale
    ...
    atteste par la présente que <span style="font-weight: bold;">« {{ strtoupper($clubName ?? '--') }} »</span> est affilié à la Fédération sous le numéro <span style="font-weight: bold;">{!! $number ?? '--' !!}</span>
    est à jour de ces cotisations pour la saison {!! $saison ?? '--' !!}.
