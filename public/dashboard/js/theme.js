...
|   Calendar
-----------------------------------------------*/
var appCalendarInit = async function appCalendarInit() {
    var Selectors = {
        ACTIVE: ".active",
        ...
        EVENT_START_DATE: '#addEventModal [name="date_debut"]',
    ...
    var events = [];
    var { data } = await axios.get("/dashboard/calendriers/data");
    data.forEach((element) => {
        events.push({
            ...
            className:
                element.type == "Formation de niveau"
                    ? "bg-primary-subtle"
                    : element.type == "Sortie formation"
                    ? "bg-success-subtle"
                    : element.type == "Sortie losir"
                    ? "bg-info-subtle"
                    : element.type == "Sortie association"
                    ? "bg-warning-subtle"
                    : "bg-danger-subtle",
            location: element.type,
            organizer: element.encadrant.nom + " " + element.encadrant.prenom,
            plongeurs: element.plongeurs
    ...
    if (appCalendar) {
        ...
        addEventForm &&
            addEventForm.addEventListener(Events.SUBMIT, async function (e) {
                e.preventDefault();
                var _e$target = e.target;
                var { data } = await axios.post(
                    "/dashboard/calendriers/ajouter",
                    {
                        titre: _e$target.titre.value,
                        date_debut: _e$target.date_debut.value,
                        date_fin: _e$target.date_fin.value,
                        type: _e$target.type.value,
                        encadrant: _e$target.encadrant.value,
                        niveau: _e$target.niveau.value,
                        exercices: Array.from(
                            _e$target.exercices.selectedOptions
                        ).map(({ value }) => value),
                        plongeurs: Array.from(
                            _e$target.plongeurs.selectedOptions
                        ).map(({ value }) => value),
                        observation: _e$target.observation.value,
                    }
                );
                const notif = `<div class="toast-container position-fixed bottom-0 end-0 p-3">
                    <div id="liveToast" class="toast" role="alert" aria-live="assertive" aria-atomic="true">
                        ...
                        <div class="toast-body">
                            Calendenrier est ajouter avec succes
                        </div>
                    </div>
                </div>`;
                ...
                calendar.addEvent({
                    title: data[0].titre,
                    start: data[0].date_debut,
                    end: data[0].date_fin,
                    className:
                        data[0].type == "Formation de niveau"
                            ? "bg-primary-subtle"
                            : data[0].type == "Sortie formation"
                            ? "bg-success-subtle"
                            : data[0].type == "Sortie losir"
                            ? "bg-info-subtle"
                            : data[0].type == "Sortie association"
                            ? "bg-warning-subtle"
                            : "bg-danger-subtle",
                    description: data[0].observation,
                    location: data[0].type,
                    organizer:
                        data[0].encadrant.nom + " " + data[0].encadrant.prenom,
                    plongeurs: data[0].plongeurs
                        .map((el) => el.nom + " " + el.prenom)
                        .join(" - "),
                });
                _e$target.titre.value = "";
                _e$target.date_debut.value = "";
                _e$target.date_fin.value = "";
                _e$target.type.value = "";
                _e$target.encadrant.value = "";
                choicesNiveau.clearStore();
                choicesPlongeurs.clearStore();
                choicesExercices.clearStore();
                choicesType.removeActiveItems();
                _e$target.observation.value = "";
                document.getElementById("exercices-element").style.display =
                    "none";
                document.getElementById("niveau-element").style.display =
                    "none";
...
var getTemplate = function getTemplate(event) {
    return '\n<div class="modal-header bg-light ps-card pe-5 border-bottom-0">\n  <div>\n    <h5 class="modal-title mb-0">'
        ...
        .concat(
            ...
            '\n    <div class="flex-1">\n        <h6>Date de debut et Date de fin</h6>\n        <p class="mb-1">\n          '
        )
        .concat(
            window.dayjs &&
        ...
        .concat(
            event.extendedProps.plongeurs
                ? '\n        <div class="d-flex mt-3">\n          '
                      .concat(
                          getStackIcon("far fa-user"),
                          '\n          <div class="flex-1">\n              <h6>Plongeurs</h6>\n              <p class="mb-0">'
                      )
                      .concat(
                          event.extendedProps.plongeurs,
                ...
                series: [
                    {
                        ...
                        data: [
                            {
                                value: document.querySelector("#countHommePlongeur").value,
                                name: "Homme",
                            },
                            {
                                value: document.querySelector("#countFemmePlongeur").value,
                                name: "Femme",
                            },
                            {
                                value: document.querySelector("#countEnfantPlongeur").value,
                                name: "Enfant",
                            },
                        ],
                    },
